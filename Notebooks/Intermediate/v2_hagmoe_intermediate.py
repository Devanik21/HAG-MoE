# -*- coding: utf-8 -*-
"""v2_HAGMoE_Intermediate.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1H9UXfZFSaVITidYptOvXHGhPkSowaltf
...
class EntropyGateV2(nn.Module):
    """Enhanced entropy gate with warm-up freeze and EMA diagnostics.

    CORRECTNESS INVARIANT:
        For all inputs: K_min ≤ k_i ≤ K_max (element-wise, unconditionally).
        At step < warmup_steps: k_i == K_min for ALL tokens (freeze period).
        After convergence of EMA: E[k_i] ≈ (K_min + K_max) / 2.

    Changes from v1 EntropyGate:
      - warmup_steps: during first warmup_steps training steps, K_i = K_min
        for all tokens. This prevents entropy statistics from being corrupted
        before the model has learned anything meaningful.
      - EMA diagnostic buffer 'ema_history' records mu_H over training
        for visualization and convergence checks.
      - 'freeze_entropy' flag allows manual override to K_min (for ablation).

    Args:
        k_min:         minimum active experts
        k_max:         maximum active experts
        alpha:         entropy gate sharpness (default 10.0)
        warmup_steps:  training steps to hold K_i = K_min (default 1000)
    """
    def __init__(self, k_min: int, k_max: int, alpha: float = 10.0,
        ...
        # EMA running mean of normalised entropy
        self.register_buffer('mu_H',         torch.tensor(0.5))
        self.register_buffer('global_step',  torch.tensor(0,   dtype=torch.long))
        self.register_buffer('ema_var',      torch.tensor(0.0))  # EMA variance
        self.momentum = 0.99
    ...
    def forward(self, attn_c: torch.Tensor, training: bool = True
        ...
        """
        Args:
            attn_c:   [B, S, S] averaged coarse attention distribution
            training: whether in training mode

        Returns:
            k_i:          [B, S] int tensor, K_i ∈ [K_min, K_max]
            norm_entropy: [B, S] float tensor, H̃_i ∈ [0, 1]
        """
        seq_len = attn_c.size(-1)
        # Shannon entropy: H = -sum_j a_j log a_j
        ...
        # Warm-up freeze: hold K_i = K_min for first warmup_steps
        ...
        # INVARIANT CHECK (debug mode only, remove for production):
        assert k_i.min().item() >= self.k_min, "K_i below K_min — invariant violated"
        assert k_i.max().item() <= self.k_max, "K_i above K_max — invariant violated"

        return k_i, norm_entropy


class RouterZLoss(nn.Module):
    """Router Z-loss: penalizes logit magnitude to prevent routing collapse.

    From: Zoph et al. (2022) "Designing Effective Sparse Expert Models"

    Loss: L_Z = (1/T) * sum_t (log sum_e exp(z_t^e))^2

    CORRECTNESS INVARIANT:
        loss >= 0 always.
        loss = 0 iff all logits are 0.
        ∂L_Z / ∂z_e = (2 * LSE) * softmax(z)_e  [verified in §1.2]

    Args:
        coarse_weight: λ_Z coefficient for coarse gate logits
        fine_weight:   λ_Z coefficient for fine gate logits
    """
    ...
    def forward(self,
                ...
                fine_logits:   Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Args:
            coarse_logits: [B, S, G] logits before softmax in CoarseGate
            fine_logits:   [B*S, M] or [T, M] logits before softmax in FineGate
        Returns:
            scalar Z-loss
        """
        loss = torch.zeros(1, device=(coarse_logits or fine_logits).device)[0]

        if coarse_logits is not None:
            # log-sum-exp over G groups, then squared
            lse_c = torch.logsumexp(coarse_logits, dim=-1)  # [B, S]
            loss  = loss + self.coarse_weight * (lse_c ** 2).mean()

        if fine_logits is not None:
            lse_f = torch.logsumexp(fine_logits, dim=-1)   # [T]
...
class ExpertImportanceEMA(nn.Module):
    """Tracks per-expert utilization via exponential moving average.

    Motivation: experts whose EMA utilization falls below a threshold
    for N_check consecutive steps are declared "dead." Dead experts
    waste parameters and can be re-initialized or merged.

    CORRECTNESS INVARIANT:
        sum_e importance[e] ≈ E[K_i] * T at stationarity
        (total expected importance = avg active experts per token * batch_size)

    Reference: Section 4.2 of "Mixture of Experts with Expert Choice Routing"
    (Zhou et al., 2022) adapts a similar EMA importance tracker.

    Args:
        num_experts: N = G * M
        momentum:    EMA decay (default 0.99)
        dead_thresh: utilization fraction below which expert is flagged dead
    """
    def __init__(self, num_experts: int, momentum: float = 0.99,
    ...
    def update(self, e_idx: torch.Tensor, s_val: torch.Tensor) -> None:
        """Update EMA importance from this step's routing decisions.

        Args:
            e_idx: [B, S, K_max] global expert indices (-1 = padding)
            s_val: [B, S, K_max] routing scores (0 for padding)
        """
        with torch.no_grad():
            freq = torch.zeros(self.num_experts, device=e_idx.device)
            valid = e_idx >= 0
            valid_idx = e_idx[valid]
            valid_score = s_val[valid]
            freq.scatter_add_(0, valid_idx, valid_score)
    ...
    def get_dead_experts(self) -> torch.Tensor:
        """Returns indices of experts below dead_thresh utilization."""
        return (self.importance < self.dead_thresh).nonzero(as_tuple=True)[0]

    def get_utilization_entropy(self) -> float:
        """Entropy of expert utilization distribution (max = log N)."""
        p = self.importance.clamp(min=1e-8)
        p = p / p.sum()
        H = -(p * p.log()).sum().item()
        return H / math.log(self.num_experts)  # normalized


class CapacityManager:
    """Manages token overflow when expert capacity is exceeded.

    In standard MoE, a capacity factor c means each expert handles at most
    ceil(c * T / N) tokens. With dynamic K_i, this must be adapted.

    Strategy: compute per-step capacity as:
        capacity_per_expert = ceil(capacity_factor * T * E[K_i] / N)

    Tokens exceeding capacity are dropped (their routing contribution = 0).
    Overflow fraction is tracked for diagnostic purposes.

    CORRECTNESS INVARIANT:
        overflow_fraction ∈ [0, 1]
        If capacity_factor >= K_max / K_min: overflow_fraction = 0 always

    Args:
        num_experts:     N = G * M
        capacity_factor: multiplier on expected load (default 1.25)
    """
    ...
    def apply_capacity(self, e_idx: torch.Tensor, s_val: torch.Tensor,
                       k_i: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, float]:
        """Enforce capacity constraint on routing assignments.

        Args:
            e_idx: [B, S, K_max] global expert indices
            s_val: [B, S, K_max] routing scores
            k_i:   [B, S] per-token cardinality

        Returns:
            e_idx_capped: same shape, -1 for dropped assignments
            s_val_capped: same shape, 0.0 for dropped assignments
            overflow_frac: fraction of (token, expert) pairs dropped
        """
        T           = e_idx.shape[0] * e_idx.shape[1]
        ...
        # Count assignments per expert (sorted by score descending to prefer high-value)
        ...
        # Sort by score descending so high-confidence assignments get priority
        ...
        return (e_out.reshape_as(e_idx),
                s_out.reshape_as(s_val),
                overflow_frac)

    def mean_overflow(self) -> float:
        return float(np.mean(self._overflow_log)) if self._overflow_log else 0.0


class GradientDetachAnalyzer(nn.Module):
    """Module that quantifies the gradient contamination caused by routing
    through un-detached attention weights.

    MOTIVATION (§7.7 of README, §HAGMoEBlock):
    The block uses attn_weights = attn_weights.detach() before computing
    c_i^c and c_i^f. Why is this critical?

    Without detach, gradients from the load-balance loss L_LB propagate
    back through the context vectors c_i^c → a^c → attn_weights → Q,K,V
    projections. This means L_LB modifies the attention mechanism, not
    just the routing — a cross-contamination that can degrade attention
    quality and destabilize training.

    This module measures the contamination magnitude by running two forward
    passes: one with detach (standard) and one without (pathological).

    CORRECTNESS INVARIANT:
        When γ=0 and K_i is constant: contamination_ratio > 0 iff L_LB > 0.
        (Routing gradients exist iff there is a load-balance loss)
    """
    @staticmethod
    def measure_contamination(model, x: torch.Tensor,
                               loss_fn) -> Dict[str, float]:
        """
        Returns:
            dict with 'Q_grad_norm_detached', 'Q_grad_norm_undetached',
            'contamination_ratio' = (undetached - detached) / detached
        """
        results = {}

        def run_and_get_Q_grad(use_detach: bool):
            set_seed(0)
            m_copy = copy.deepcopy(model)
            for block in m_copy.layers:
                block._use_detach = use_detach
            x_clone = x.detach().clone().requires_grad_(False)
            logits, aux = m_copy(x_clone)
            loss = loss_fn(logits, aux)
            loss.backward()
            # Return Q-projection gradient norm for first layer
            q_grad = m_copy.layers[0].attn.q_proj.weight.grad
            return q_grad.norm().item() if q_grad is not None else 0.0

        # This requires patching the block; done in HAGMoEBlockV2 below
        # For now, return placeholder structure
        return {'Q_grad_norm_detached': 0.0,
                'Q_grad_norm_undetached': 0.0,
                'contamination_ratio': 0.0}


class CKASimilarity:
    """Centered Kernel Alignment: measures representational similarity
    between two sets of representations.

    CKA(X, Y) = HSIC(X, Y) / sqrt(HSIC(X, X) * HSIC(Y, Y))

    where HSIC is the Hilbert-Schmidt Independence Criterion with linear kernel.

    PROPERTIES:
        CKA ∈ [0, 1]
        CKA(X, X) = 1
        CKA is invariant to orthogonal transformations and scaling
        CKA = 0 iff X and Y are statistically independent (linear HSIC)

    We use CKA to measure:
      1. Similarity between coarse and fine context vectors (should decrease with training)
      2. Similarity between expert group outputs (should be diverse)
      3. Cross-layer routing similarity (measures depth of specialization)

    Reference: Kornblith et al. (2019) "Similarity of Neural Network Representations
    Revisited" (NeurIPS 2019).
    """
    @staticmethod
    def linear_cka(X: np.ndarray, Y: np.ndarray) -> float:
        """Compute linear CKA between X [n, d1] and Y [n, d2].

        CORRECTNESS INVARIANT: returns value in [0, 1]
        """
        def center(K):
            n = K.shape[0]
    ...
    @staticmethod
    def batch_cka(Xs: List[np.ndarray], Ys: List[np.ndarray],
                  max_samples: int = 512) -> float:
        """Average CKA over multiple batches."""
        ckas = []
        for X, Y in zip(Xs, Ys):
            n = min(X.shape[0], Y.shape[0], max_samples)
            idx = np.random.choice(X.shape[0], n, replace=False)
            ckas.append(CKASimilarity.linear_cka(X[idx], Y[idx]))
        return float(np.mean(ckas))


# ── Verify CKA invariants ─────────────────────────────────────────────────────
def verify_cka_invariants():
    """Property-based tests for CKA implementation."""
    rng = np.random.default_rng(0)
    n, d = 200, 64
    X = rng.standard_normal((n, d))

    # CKA(X, X) = 1
    cka_self = CKASimilarity.linear_cka(X, X)
    assert abs(cka_self - 1.0) < 1e-6, f"CKA(X,X)={cka_self} != 1.0"
    print(f"CKA invariant 1 (self-similarity=1): {cka_self:.6f} ✓")

    # CKA ∈ [0,1] for random pairs
    Y = rng.standard_normal((n, d))
    cka_rand = CKASimilarity.linear_cka(X, Y)
    assert 0.0 <= cka_rand <= 1.0, f"CKA out of range: {cka_rand}"
    print(f"CKA invariant 2 (range [0,1]):       {cka_rand:.6f} ✓")

    # CKA invariant to orthogonal transform
    Q, _ = np.linalg.qr(rng.standard_normal((d, d)))
    cka_rot = CKASimilarity.linear_cka(X, X @ Q)
    assert abs(cka_rot - 1.0) < 1e-5, f"CKA not rotation-invariant: {cka_rot}"
    print(f"CKA invariant 3 (rotation-invariant): {cka_rot:.6f} ✓")

    print("All CKA invariants passed.")

verify_cka_invariants()
...
CFG = dict(
    ...
    max_seq_len    = SEQ_LEN,
    capacity_factor = 1.5,
    warmup_steps   = 300,    # entropy gate freeze (first 300 steps)
    # training
    lr             = 3e-4,
    weight_decay   = 0.1,
    grad_clip      = 1.0,
...
def cosine_lr_with_warmup(step: int, warmup: int, total: int, max_lr: float,
    ...
    """Standard cosine LR with linear warmup.

    During warmup: lr = max_lr * step / warmup
    After warmup:  lr = min_lr + 0.5 * (max_lr - min_lr) * (1 + cos(π * progress))

    CORRECTNESS INVARIANT:
        lr(0) = 0  (no learning at step 0, gradients well-conditioned)
        lr(warmup) = max_lr
        lr(total) = min_lr_frac * max_lr
    """
...
#   - Biases (including LayerNorm bias)
#   - LayerNorm weight (scale)
#   - Scalar parameters (gamma, entropy gate mu_H buffer)
#
# Motivation: weight decay on biases → systematic shrinkage of learned shifts.
# Weight decay on LN weight → compresses the representation scale, which
# interacts badly with initialization schemes (He, Xavier).
...
def evaluate_model(model, val_loader, criterion, lam_sched, n_batches=25):
    """Run validation and return loss dict + CKA scores."""
    model.eval()
    loss_accum = defaultdict(float)
    cka_cc_cf_scores = defaultdict(list)  # layer → list of CKA(c^c, c^f)
    util_entropies   = defaultdict(list)  # layer → utilization entropy
    overflow_fracs   = []
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            with torch.cuda.amp.autocast(enabled=USE_AMP, dtype=AMP_DTYPE):
                ...
                loss, ld = criterion(logits[:, :seq_len], lbls[:, :seq_len],
                                     all_aux, layer_lambdas)

            for k, v in ld.items():
                loss_accum[k] += v

            # CKA(c^c, c^f) per layer
            for l_idx, aux in enumerate(all_aux):
                cc = aux['c_i_c'].cpu().numpy().reshape(-1, CFG['d_model'])  # [B*S, d]
                cf = aux['c_i_f'].cpu().numpy().reshape(-1, CFG['d_model'])
...
print(f"  AMP: {AMP_DTYPE} | device: {DEVICE}")
print("=" * 65)

# Fresh trained model for δ-comparison (head divergence section)
...
# Gradient accumulation: accumulate GRAD_ACCUM steps before optimizer.step()
optimizer.zero_grad()
...
while step < CFG['total_steps']:
    ...
    # LR update
    lr = cosine_lr_with_warmup(step, CFG['lr_warmup_steps'],
                                CFG['total_steps'], CFG['lr'])
    ...
    mean_k     = float(np.mean([aux['k_i'].float().mean().item() for aux in all_aux]))
    mean_H     = float(np.mean([aux['norm_entropy'].mean().item() for aux in all_aux]))
    mean_z     = float(np.mean([aux['z_loss'].item() for aux in all_aux]))
    mean_util  = float(np.mean([aux['util_entropy'] for aux in all_aux]))
    overflow   = float(np.mean([aux['overflow_frac'] for aux in all_aux]))
    ...
    history['tr_z'].append(accum_ld['z'])
    ...
    history['z_loss'].append(mean_z)
    history['util_entropy'].append(mean_util)
    history['overflow'].append(overflow)
    history['total_gnorm'].append(float(total_gnorm))
    for tag in module_grad_tags:
        history[f'gnorm_{tag}'].append(grad_norms.get(tag, 0.0))

    # ── Evaluation ────────────────────────────────────────────────────────────
    if step % CFG['eval_every'] == 0 or step == CFG['total_steps']:
        val_losses, val_cka, val_util, val_overflow = evaluate_model(
            model, val_loader, criterion, lam_sched)
        ...
        for l_idx in range(CFG['num_layers']):
            history[f'val_cka_layer{l_idx}'].append(val_cka.get(l_idx, 0.0))
            history[f'val_util_layer{l_idx}'].append(val_util.get(l_idx, 0.0))
        ...
        print(f"  step={step:5d} | lr={lr:.2e} | "
              f"tr_lm={accum_ld['lm']:.4f} | val_lm={val_losses['lm']:.4f} | "
              f"ppl={val_ppl:.1f} | γ={np.mean([abs(g) for g in gammas]):.4f} | "
              f"K̄={mean_k:.2f} | Ēntropy={mean_H:.3f} | "
              f"z={mean_z:.4f} | overflow={overflow:.3f} | "
              f"t={elapsed:.0f}s")
...
ax.set_title('LM Loss (Cross-Entropy)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('CE Loss'); ax.legend()

# Panel 1.2: Load balance losses
ax = fig.add_subplot(gs[0, 1])
ax.plot(steps, smooth(history['tr_lb_g']), lw=2, color='#1565C0', label='LB group')
ax.plot(steps, smooth(history['tr_lb_e']), lw=2, color='#C62828', label='LB expert')
ax.set_title('Load Balance Losses\n(should decrease → balanced routing)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Auxiliary Loss'); ax.legend()

# Panel 1.3: Divergence + Gamma regularizer
ax = fig.add_subplot(gs[0, 2])
ax2 = ax.twinx()
ax.plot(steps,  smooth(history['tr_div']),        lw=2, color=PAL['kl'],    label='L_div')
ax2.plot(steps, smooth(history['tr_gamma_loss']), lw=2, color=PAL['gamma'], ls='--', label='L_γ')
ax.set_title('Divergence + Gamma Regularizer', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('L_div', color=PAL['kl'])
ax2.set_ylabel('L_γ', color=PAL['gamma'])
ax.legend(loc='upper right'); ax2.legend(loc='upper left')

# Panel 1.4: Z-loss
ax = fig.add_subplot(gs[0, 3])
ax.plot(steps, smooth(history['z_loss']), lw=2, color='#4E342E')
ax.fill_between(steps, smooth(history['z_loss']), 0, alpha=0.2, color='#4E342E')
ax.set_title('Router Z-Loss\n(anchors logits near 0)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Z-Loss Value')
ax.axhline(0, color='green', ls='--', lw=1.0, alpha=0.5)

# ── Row 2: Routing Diagnostics ────────────────────────────────────────────────
# Panel 2.1: Mean K_i over training
ax = fig.add_subplot(gs[1, 0])
ax.plot(steps, smooth(history['mean_k']), lw=2, color=PAL['k_i'])
ax.axhline(CFG['k_min'], ls='--', color='blue', lw=1.2, label=f'K_min={CFG["k_min"]}')
ax.axhline(CFG['k_max'], ls='--', color='red',  lw=1.2, label=f'K_max={CFG["k_max"]}')
ax.axhline((CFG['k_min']+CFG['k_max'])/2, ls=':', color='gray', lw=1.0, label='Midpoint')
# Mark warmup end
ax.axvline(CFG['warmup_steps']/GRAD_ACCUM, color='orange', ls=':', lw=1.5, label='Warmup end')
ax.set_title('Mean K_i Over Training\n(entropy gate activates post-warmup)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Mean Active Experts K̄'); ax.legend(fontsize=7.5)

# Panel 2.2: Normalised entropy over training
ax = fig.add_subplot(gs[1, 1])
ax.plot(steps, smooth(history['mean_H']), lw=2, color=PAL['entropy'])
ax.fill_between(steps, smooth(history['mean_H']), 0, alpha=0.15, color=PAL['entropy'])
ax.axhline(0.5, ls='--', color='gray', lw=1.2, label='μ_H target (EMA)')
ax.set_title('Mean Normalised Entropy H̃\n(EMA target is 0.5)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('H̃ = H / log(S)'); ax.legend()

# Panel 2.3: Token overflow fraction
ax = fig.add_subplot(gs[1, 2])
ax.plot(steps, smooth(history['overflow'], w=10), lw=2, color='#AD1457')
ax.axhline(0.05, ls='--', color='red', lw=1.2, label='5% threshold (acceptable)')
ax.fill_between(steps, smooth(history['overflow'], w=10), 0, alpha=0.15, color='#AD1457')
ax.set_title('Token Overflow Fraction\n(capacity_factor=1.5)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Overflow Fraction'); ax.legend()

# Panel 2.4: Expert utilization entropy
ax = fig.add_subplot(gs[1, 3])
ax.plot(steps, smooth(history['util_entropy'], w=10), lw=2, color=PAL['damoe'])
ax.axhline(1.0, ls='--', color='green', lw=1.2, label='Max entropy (uniform)')
ax.set_title('Expert Utilization Entropy\n(1.0 = perfectly balanced)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Normalized H_util'); ax.legend()

# ── Row 3: Gradient Health ────────────────────────────────────────────────────
# Panel 3.1: Total gradient norm
ax = fig.add_subplot(gs[2, 0])
ax.semilogy(steps, smooth(history['total_gnorm'], w=10), lw=2, color='black')
ax.axhline(CFG['grad_clip'], ls='--', color='red', lw=1.2, label=f'clip={CFG["grad_clip"]}')
ax.set_title('Total Gradient Norm\n(clipped)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('||∇||_2 (log scale)'); ax.legend()

# Panel 3.2: Per-module gradient norms
...
for tag, c in zip(module_grad_tags, colors_modules):
    ...
    if vals:
        ax.plot(steps[:len(vals)], smooth(vals, w=8), lw=1.8, color=c, label=tag)
ax.set_title('Per-Module Gradient Norms\n(routing vs experts vs feedback)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('Module ||∇||_2'); ax.legend(fontsize=7.5)

# Panel 3.3: γ per layer
ax = fig.add_subplot(gs[2, 2])
gamma_per_layer = np.array(history['gamma_per_layer'])  # [steps, L]
colors_layers = plt.cm.Blues(np.linspace(0.4, 0.9, CFG['num_layers']))
for l in range(CFG['num_layers']):
    if gamma_per_layer.shape[0] > 0:
        ax.plot(steps, np.abs(gamma_per_layer[:, l]), lw=2, color=colors_layers[l],
                label=f'Layer {l}')
ax.axhline(0, color='red', ls='--', lw=1, label='γ=0 (init)')
ax.set_title('|γ| Per Layer Over Training\n(grows from 0 = feedback activated)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('|γ|'); ax.legend(fontsize=8)

# Panel 3.4: Learning rate curve
ax = fig.add_subplot(gs[2, 3])
lr_curve = [cosine_lr_with_warmup(s, CFG['lr_warmup_steps'],
                                   CFG['total_steps'], CFG['lr'])
            for s in range(1, CFG['total_steps'] + 1)]
ax.plot(range(1, CFG['total_steps'] + 1), lr_curve, lw=2.5, color=PAL['lr'])
ax.axvline(CFG['lr_warmup_steps'], color='orange', ls=':', lw=1.5, label='Warmup end')
ax.set_title('Learning Rate Schedule\n(linear warmup + cosine decay)', fontweight='bold')
ax.set_xlabel('Step'); ax.set_ylabel('LR'); ax.legend()

fig.suptitle('HAG-MoE v2 — Training Dynamics: 12-Panel Diagnostic Dashboard',
             fontsize=13, fontweight='bold', y=1.01)
plt.savefig('fig_v2_training_dynamics.png', dpi=150, bbox_inches='tight')
plt.show()

# ── CKA evolution: coarse vs fine context vectors ────────────────────────────
fig, axes = plt.subplots(1, CFG['num_layers'], figsize=(6 * CFG['num_layers'], 5))
if CFG['num_layers'] == 1:
    axes = [axes]
for l in range(CFG['num_layers']):
    key  = f'val_cka_layer{l}'
    vals = history.get(key, [])
    ax   = axes[l]
    ax.plot(val_steps[:len(vals)], vals, 'o-', lw=2, ms=6, color=PAL['hag'])
    ax.axhline(1.0, ls='--', color='gray', lw=1, label='CKA=1 (identical)')
    ax.axhline(0.0, ls='--', color='red',  lw=1, label='CKA=0 (independent)')
    ax.set_title(f'Layer {l}: CKA(c^c, c^f)\n(should decrease → head specialization)',
                 fontweight='bold')
    ax.set_xlabel('Training Step'); ax.set_ylabel('Linear CKA')
    ax.set_ylim(-0.05, 1.05); ax.legend(fontsize=8)
fig.suptitle('CKA(Coarse Context, Fine Context) Over Training\n'
             'Decreasing CKA = coarse/fine representations diverging (supports Claim 3)',
             fontsize=11, fontweight='bold')
plt.tight_layout()
plt.savefig('fig_v2_cka_evolution.png', dpi=150, bbox_inches='tight')
plt.show()
...
MICRO_CFG = dict(
    ...
    capacity_factor=1.5, warmup_steps=80)
...
SMALL_CFG = dict(
    ...
    capacity_factor=1.5, warmup_steps=300)  # identical to CFG
